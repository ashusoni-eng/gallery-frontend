...
export function EditCategoryDialog({
  ...
  useEffect(() => {
    ...
    if (open && !dlg.open) {
      ...
      dlg.style.top = `calc(50% - ${dialogDimensions.height / 2}px)`
      dlg.style.left = `calc(50% - ${dialogDimensions.width / 2}px)`
    }
  ...
  }, [open, current, currentDescription])
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <dialog
            ...
            className="rounded-lg border bg-background p-0 shadow-lg w-[min(96vw,480px)]"
          >
            <form method="dialog">
              <div className="border-b p-4">
                <div className="text-lg font-semibold">Edit Category</div>
                <div className="text-sm text-muted-foreground">Update the category name and description, and save changes.</div>
              </div>
              <div className="p-4 space-y-4">
                ...
                />
                <textarea
                  className="flex h-20 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-xs outline-none focus-visible:ring-2 focus-visible:ring-ring/50"
                  value={description}
